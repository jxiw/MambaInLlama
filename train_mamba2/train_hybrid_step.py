...
def main():
    ...
    # load dataset
    progressive_step = training_args.progressive_step
    total_progressive_step = training_args.total_progressive_step

    train_data = torch.cat([torch.chunk(torch.load(f'{train_dataset_path}/input_ids.pt', map_location="cpu"), total_progressive_step)[progressive_step] for train_dataset_path in training_args.train_datasets_path], dim=0)
    train_label = torch.cat([torch.chunk(torch.load(f'{train_dataset_path}/labels.pt', map_location="cpu"), total_progressive_step)[progressive_step] for train_dataset_path in training_args.train_datasets_path], dim=0)
