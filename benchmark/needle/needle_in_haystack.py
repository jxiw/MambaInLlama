...
class LLMNeedleHaystackTester:
    ...
    def __init__(self,
        ...
        if(self.model_provider not in ["OpenAI", "Anthropic"]):
            ...
            if "mamba2" in test_name.lower():
                print("mamba2 model_name 2:", model_name)
                from mamba2.hybrid_wrapper import MambaTransformerHybridModelWrapper
                self.model_to_test = MambaTransformerHybridModelWrapper.from_pretrained(model_name, torch_dtype=torch.bfloat16).model.cuda()
                self.model_to_test.config.use_cache = False
                self.model_to_test.generation_config.use_cache = False
                print(self.model_to_test)
                print("*********")
            elif "mamba" in test_name.lower():
                print("mamba model_name 2:", model_name)
    ...
    def encode_text_to_tokens(self, text):
        if self.model_provider in ["Mamba", "OpenAI", "LLaMA", "Mistral", "GLM"]:
    ...
    def insert_needle(self, context, depth_percent, context_length):
        ...
        if depth_percent == 100:
            ...
            if(self.model_provider in ["Mamba"]): period_tokens = [13, 662]
